...
| `sd_spi_host.sv` | **纯 SD SPI 主机**：`sck/mosi/miso/cs_n`，无 x86 I/O 端口；块读/写请求为占位 FSM，可扩展为完整 CMD/DATA。 |
...
- `disk_ram_8_tb.sv`：根目录 `./test_all_modules.sh` 会自动编译 `sd_disk_ram_8.sv` + `ide_ata_pio.sv`。
