...
module eu_misc_exchange_status_tb;
    ...
    initial begin
        ...
        if (flags_out[7] !== eax_in[15] ||
            ...
            $display("FAIL eu_misc_sahf flag bits");
            $finish(1);
        end
        if (flags_out[11] !== flags_in[11] || flags_out[1] !== flags_in[1]) begin
            $display("FAIL eu_misc_sahf preserved bits");
            $finish(1);
        end
        ...
        if (cmpxchg_zf !== 1'b1 || cmpxchg_y !== 32'hAABB_CCDD) begin
            $display("FAIL eu_misc_cmpxchg equal path");
            $finish(1);
        end
        ...
        if (cmpxchg_zf !== 1'b0 || cmpxchg_y !== 32'h0000_1234) begin
            $display("FAIL eu_misc_cmpxchg mismatch path");
            $finish(1);
        end
