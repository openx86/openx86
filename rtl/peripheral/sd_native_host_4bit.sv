...
module sd_native_host_4bit (
    ...
    output logic         o_sd_native_host_4bit_phy_clk,
    output logic         o_sd_native_host_4bit_phy_cmd_out,
    output logic         o_sd_native_host_4bit_phy_cmd_oe,
    input  logic         i_sd_native_host_4bit_phy_cmd_in,
    output logic [ 3: 0] o_sd_native_host_4bit_phy_dat_out,
    output logic         o_sd_native_host_4bit_phy_dat_oe,
    input  logic [ 3: 0] i_sd_native_host_4bit_phy_dat_in,
    input  logic         clock,
    input  logic         reset_n
);
    ...
    assign o_sd_native_host_4bit_phy_clk = clock;
    ...
    always_ff @(posedge clock or negedge reset_n) begin
        if (~reset_n) begin
            ...
            o_payload_addr <= '0;
            o_payload_data <= '0;
            o_sd_native_host_4bit_phy_cmd_out <= 1'b1;
            o_sd_native_host_4bit_phy_cmd_oe  <= 1'b0;
            o_sd_native_host_4bit_phy_dat_out <= 4'hF;
            o_sd_native_host_4bit_phy_dat_oe  <= 1'b0;
        end else begin
            o_done   <= 1'b0;
            o_err    <= 1'b0;
            o_payload_we <= 1'b0;
            ...
            unique case (state)
                ...
                ST_LATCH_LBA: begin
                    ...
                    send_bit_ix <= '0;
                    state       <= ST_SEND_CMD;
                    o_sd_native_host_4bit_phy_cmd_oe <= 1'b1;
                end

                ST_SEND_CMD: begin
                    o_sd_native_host_4bit_phy_cmd_out <= cmd_frame[47 - send_bit_ix];
                    o_sd_native_host_4bit_phy_cmd_oe  <= 1'b1;
                ...
                ST_WAIT_TOKEN_E: begin
                    if (i_sd_native_host_4bit_phy_dat_in == 4'hE) begin
                        state   <= ST_READ_LO;
                        byte_ix <= '0;
                    end
                end
                ...
                ST_READ_LO: begin
                    lo_nib <= i_sd_native_host_4bit_phy_dat_in;
                    state  <= ST_READ_HI;
                end
                ...
                ST_READ_HI: begin
                    ...
                    o_payload_data <= { i_sd_native_host_4bit_phy_dat_in, lo_nib };
                    if (byte_ix == 9'd511)
                        state <= ST_FINISH;
                    else begin
