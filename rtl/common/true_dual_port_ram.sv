...
module true_dual_port_ram #(
    ...
    input  logic [P_ADDR_WIDTH-1: 0] addra, // A 口地址（读/写共用）
    input  logic [P_DATA_WIDTH-1: 0] wdataa, // A 口写数据
    output logic [P_DATA_WIDTH-1: 0] rdataa, // A 口同步读输出

    // 端口B（通常用于VGA读取）
    input  logic                  web,     // B 口写使能；只读场景可常接 0
    input  logic [P_ADDR_WIDTH-1: 0] addrb, // B 口地址
    input  logic [P_DATA_WIDTH-1: 0] wdatab, // B 口写数据
    output logic [P_DATA_WIDTH-1: 0] rdatab, // B 口同步读输出
