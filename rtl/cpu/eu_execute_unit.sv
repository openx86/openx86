...
module eu_execute_unit (
    ...
    eu_ari_add u_int_add (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_add_res )
    );

    eu_ari_adc u_int_adc (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .cf ( i_int_cf ),
        .y ( int_adc_res )
    ...
    eu_ari_sub u_int_sub (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_sub_res )
    );

    eu_ari_sbb u_int_sbb (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .cf ( i_int_cf ),
        .y ( int_sbb_res )
    ...
    eu_log_xor u_int_xor (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_xor_res )
    );

    eu_log_not u_int_not (
        .a ( i_int_a ),
        .y ( int_not_res )
    );

    eu_ari_neg u_int_neg (
        .a ( i_int_a ),
        .y ( int_neg_res )
    );

    eu_ari_inc u_int_inc (
        .a ( i_int_a ),
        .y ( int_inc_res )
    );

    eu_ari_dec u_int_dec (
        .a ( i_int_a ),
        .y ( int_dec_res )
    );

    eu_shf_shl u_int_shl (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_shl_res )
    );

    eu_shf_shr u_int_shr (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_shr_res )
    );

    eu_shf_sar u_int_sar (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_sar_res )
    );

    eu_shf_shld u_int_shld (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .count ( i_int_count ),
        .y ( int_shld_res )
    ...
    eu_rot_rol u_int_rol (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_rol_res )
    );

    eu_rot_ror u_int_ror (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_ror_res )
    );

    eu_rot_rcl u_int_rcl (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .cf_in ( i_int_cf ),
        .y ( int_rcl_res ),
    ...
    eu_bit_bsf u_int_bsf (
        .a ( i_int_a ),
        .y ( int_bsf_res ),
        .zf ( int_bsf_zf )
    );

    eu_bit_bsr u_int_bsr (
        .a ( i_int_a ),
        .y ( int_bsr_res ),
        .zf ( int_bsr_zf )
    );
    ...
    eu_misc_bswap u_int_bswap (
        .a ( i_int_a ),
        .y ( int_bswap_res )
    );

    eu_misc_aaa u_int_aaa (
        .a ( i_int_a ),
        .af_in ( i_int_af ),
        .y ( int_aaa_res ),
        .af_out ( int_aaa_af ),
    ...
    eu_misc_aad u_int_aad (
        .a ( i_int_a ),
        .y ( int_aad_res )
    );

    eu_misc_aam u_int_aam (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_aam_res )
    );

    eu_misc_cbw u_int_cbw (
        .a ( i_int_a ),
        .y ( int_cbw_res )
    );

    eu_misc_cdq u_int_cdq (
        .a ( i_int_a ),
        .y ( int_cdq_res )
    );

    eu_misc_movsx u_int_movsx (
        .a ( i_int_a ),
        .width ( i_int_count[1:0] ),
        .y ( int_movsx_res )
    );

    eu_misc_movzx u_int_movzx (
        .a ( i_int_a ),
        .width ( i_int_count[1:0] ),
        .y ( int_movzx_res )
    );

    eu_misc_lahf u_int_lahf (
        .eax_in ( i_int_a ),
        .flags_in ( i_int_b ),
        .eax_out ( int_lahf_res )
    );

    eu_misc_sahf u_int_sahf (
        .flags_in ( i_int_a ),
        .eax_in ( i_int_b ),
        .flags_out ( int_sahf_res )
    );

    eu_misc_xchg u_int_xchg (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_xchg_res )
    );

    eu_misc_xadd u_int_xadd (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_xadd_res )
    );

    eu_misc_cmpxchg u_int_cmpxchg (
        .acc ( i_int_a ),
        .dst ( i_int_b ),
        .src ( i_int_count ),
        .y ( int_cmpxchg_res ),
    ...
    eu_misc_setcc u_int_setcc (
        .flags ( i_int_a ),
        .tttn ( i_int_count[3:0] ),
        .y ( int_setcc_res )
    );

    eu_misc_arpl u_int_arpl (
        .dst ( i_int_a ),
        .src ( i_int_b ),
        .y ( int_arpl_res ),
        .zf ( int_arpl_zf )
    ...
    eu_misc_lar u_int_lar (
        .src ( i_int_a ),
        .y ( int_lar_res ),
        .zf ( int_lar_zf )
    );

    eu_misc_lsl u_int_lsl (
        .src ( i_int_a ),
        .y ( int_lsl_res ),
        .zf ( int_lsl_zf )
    );

    eu_misc_verr u_int_verr (
        .selector ( i_int_a ),
        .zf ( int_verr_zf )
    );

    eu_misc_stridx_step u_int_stridx_step (
        .idx ( i_int_a ),
        .df ( i_int_count[0] ),
        .y ( int_stridx_step_res )
    );

    eu_misc_imul_imm u_int_imul_imm (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_imul_imm_res ),
    ...
    eu_misc_clts u_int_clts (
        .cr0 ( i_int_a ),
        .y ( int_clts_res )
    );

    eu_misc_lmsw u_int_lmsw (
        .cr0 ( i_int_a ),
        .src ( i_int_b ),
        .y ( int_lmsw_res )
    );

    eu_misc_smsw u_int_smsw (
        .cr0 ( i_int_a ),
        .y ( int_smsw_res )
    );

    eu_misc_loop_ctrl u_int_loop_ctrl (
        .ecx ( i_int_a ),
        .zf ( i_int_b[0] ),
    ...
    eu_misc_flag_status u_int_flag_status (
        .flags_in ( i_int_a ),
        .op ( i_int_op ),
    ...
    always_comb begin
        ...
        if (i_int_valid) begin
            unique case (i_int_op)
                INT_ADD: o_int_result = int_add_res;
                INT_ADC: o_int_result = int_adc_res;
                INT_SUB: o_int_result = int_sub_res;
                INT_SBB: o_int_result = int_sbb_res;
                INT_AND: o_int_result = int_and_res;
                INT_OR:  o_int_result = int_or_res;
                INT_XOR: o_int_result = int_xor_res;
                INT_NOT: o_int_result = int_not_res;
                INT_NEG: o_int_result = int_neg_res;
                INT_INC: o_int_result = int_inc_res;
                INT_DEC: o_int_result = int_dec_res;
                INT_SHL: o_int_result = int_shl_res;
                INT_SHR: o_int_result = int_shr_res;
                INT_SAR: o_int_result = int_sar_res;
                INT_SHLD: o_int_result = int_shld_res;
                INT_SHRD: o_int_result = int_shrd_res;
                INT_ROL: o_int_result = int_rol_res;
                INT_ROR: o_int_result = int_ror_res;
                INT_RCL: begin
