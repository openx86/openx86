// ============================================================================
// stage_3_exe_eu_execute_unit — AGU / Branch / MulDiv / X87 子模块聚合顶层
// 译码/微码侧通过选择信号驱动各簇；此处为直连端口便于 SoC 集成
// ============================================================================

module stage_3_exe_eu_execute_unit (
    input logic clk,
    input logic rst,
    ...
    stage_3_exe_eu_address_generation_unit u_agu (
        .i_base              ( i_agu_base ),
        .i_index             ( i_agu_index ),
        .i_scale             ( i_agu_scale ),
        .i_disp              ( i_agu_disp ),
    ...
    stage_3_exe_eu_execute_branch_unit u_br (
    ...
    stage_3_exe_eu_execute_muldiv_unit u_md (
        .i_op   ( i_md_op ),
        .i_lo   ( i_md_lo ),
        .i_hi   ( i_md_hi ),
        .i_src  ( i_md_src ),
    ...
    stage_3_exe_eu_ari_add u_int_add (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_add_res )
    );

    stage_3_exe_eu_ari_adc u_int_adc (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .cf ( i_int_cf ),
        .y ( int_adc_res )
    ...
    stage_3_exe_eu_ari_sub u_int_sub (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_sub_res )
    );

    stage_3_exe_eu_ari_sbb u_int_sbb (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .cf ( i_int_cf ),
        .y ( int_sbb_res )
    ...
    stage_3_exe_eu_log_and u_int_and (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_and_res )
    );

    stage_3_exe_eu_log_or u_int_or (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_or_res )
    );

    stage_3_exe_eu_log_xor u_int_xor (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_xor_res )
    );

    stage_3_exe_eu_log_not u_int_not (
        .a ( i_int_a ),
        .y ( int_not_res )
    );

    stage_3_exe_eu_ari_neg u_int_neg (
        .a ( i_int_a ),
        .y ( int_neg_res )
    );

    stage_3_exe_eu_ari_inc u_int_inc (
        .a ( i_int_a ),
        .y ( int_inc_res )
    );

    stage_3_exe_eu_ari_dec u_int_dec (
        .a ( i_int_a ),
        .y ( int_dec_res )
    );

    stage_3_exe_eu_shf_shl u_int_shl (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_shl_res )
    );

    stage_3_exe_eu_shf_shr u_int_shr (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_shr_res )
    );

    stage_3_exe_eu_shf_sar u_int_sar (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_sar_res )
    );

    stage_3_exe_eu_shf_shld u_int_shld (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .count ( i_int_count ),
        .y ( int_shld_res )
    ...
    stage_3_exe_eu_shf_shrd u_int_shrd (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .count ( i_int_count ),
        .y ( int_shrd_res )
    ...
    stage_3_exe_eu_rot_rol u_int_rol (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_rol_res )
    );

    stage_3_exe_eu_rot_ror u_int_ror (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .y ( int_ror_res )
    );

    stage_3_exe_eu_rot_rcl u_int_rcl (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .cf_in ( i_int_cf ),
        .y ( int_rcl_res ),
    ...
    stage_3_exe_eu_rot_rcr u_int_rcr (
        .a ( i_int_a ),
        .count ( i_int_count ),
        .cf_in ( i_int_cf ),
        .y ( int_rcr_res ),
    ...
    stage_3_exe_eu_bit_bsf u_int_bsf (
        .a ( i_int_a ),
        .y ( int_bsf_res ),
        .zf ( int_bsf_zf )
    );

    stage_3_exe_eu_bit_bsr u_int_bsr (
        .a ( i_int_a ),
        .y ( int_bsr_res ),
        .zf ( int_bsr_zf )
    );

    stage_3_exe_eu_bit_bt u_int_bt (
        .a ( i_int_a ),
        .bit_index ( i_int_b ),
        .y ( int_bt_res ),
        .cf ( int_bt_cf )
    ...
    stage_3_exe_eu_bit_bts u_int_bts (
        .a ( i_int_a ),
        .bit_index ( i_int_b ),
        .y ( int_bts_res ),
        .cf ( int_bts_cf )
    ...
    stage_3_exe_eu_bit_btr u_int_btr (
        .a ( i_int_a ),
        .bit_index ( i_int_b ),
        .y ( int_btr_res ),
        .cf ( int_btr_cf )
    ...
    stage_3_exe_eu_bit_btc u_int_btc (
        .a ( i_int_a ),
        .bit_index ( i_int_b ),
        .y ( int_btc_res ),
        .cf ( int_btc_cf )
    ...
    stage_3_exe_eu_misc_bswap u_int_bswap (
        .a ( i_int_a ),
        .y ( int_bswap_res )
    );

    stage_3_exe_eu_misc_aaa u_int_aaa (
        .a ( i_int_a ),
        .af_in ( i_int_af ),
        .y ( int_aaa_res ),
        .af_out ( int_aaa_af ),
    ...
    stage_3_exe_eu_misc_aas u_int_aas (
        .a ( i_int_a ),
        .af_in ( i_int_af ),
        .y ( int_aas_res ),
        .af_out ( int_aas_af ),
    ...
    stage_3_exe_eu_misc_daa u_int_daa (
        .a ( i_int_a ),
        .af_in ( i_int_af ),
        .cf_in ( i_int_cf ),
        .y ( int_daa_res ),
    ...
    stage_3_exe_eu_misc_das u_int_das (
        .a ( i_int_a ),
        .af_in ( i_int_af ),
        .cf_in ( i_int_cf ),
        .y ( int_das_res ),
    ...
    stage_3_exe_eu_misc_aad u_int_aad (
        .a ( i_int_a ),
        .y ( int_aad_res )
    );

    stage_3_exe_eu_misc_aam u_int_aam (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_aam_res )
    );

    stage_3_exe_eu_misc_cbw u_int_cbw (
        .a ( i_int_a ),
        .y ( int_cbw_res )
    );

    stage_3_exe_eu_misc_cdq u_int_cdq (
        .a ( i_int_a ),
        .y ( int_cdq_res )
    );

    stage_3_exe_eu_misc_movsx u_int_movsx (
        .a ( i_int_a ),
        .width ( i_int_count[1:0] ),
        .y ( int_movsx_res )
    );

    stage_3_exe_eu_misc_movzx u_int_movzx (
        .a ( i_int_a ),
        .width ( i_int_count[1:0] ),
        .y ( int_movzx_res )
    );

    stage_3_exe_eu_misc_lahf u_int_lahf (
        .eax_in ( i_int_a ),
        .flags_in ( i_int_b ),
        .eax_out ( int_lahf_res )
    );

    stage_3_exe_eu_misc_sahf u_int_sahf (
        .flags_in ( i_int_a ),
        .eax_in ( i_int_b ),
        .flags_out ( int_sahf_res )
    );

    stage_3_exe_eu_misc_xchg u_int_xchg (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_xchg_res )
    );

    stage_3_exe_eu_misc_xadd u_int_xadd (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_xadd_res )
    );

    stage_3_exe_eu_misc_cmpxchg u_int_cmpxchg (
        .acc ( i_int_a ),
        .dst ( i_int_b ),
        .src ( i_int_count ),
        .y ( int_cmpxchg_res ),
    ...
    stage_3_exe_eu_misc_setcc u_int_setcc (
        .flags ( i_int_a ),
        .tttn ( i_int_count[3:0] ),
        .y ( int_setcc_res )
    );

    stage_3_exe_eu_misc_arpl u_int_arpl (
        .dst ( i_int_a ),
        .src ( i_int_b ),
        .y ( int_arpl_res ),
        .zf ( int_arpl_zf )
    ...
    stage_3_exe_eu_misc_lar u_int_lar (
        .src ( i_int_a ),
        .y ( int_lar_res ),
        .zf ( int_lar_zf )
    );

    stage_3_exe_eu_misc_lsl u_int_lsl (
        .src ( i_int_a ),
        .y ( int_lsl_res ),
        .zf ( int_lsl_zf )
    );

    stage_3_exe_eu_misc_verr u_int_verr (
        .selector ( i_int_a ),
        .zf ( int_verr_zf )
    );

    stage_3_exe_eu_misc_stridx_step u_int_stridx_step (
        .idx ( i_int_a ),
        .df ( i_int_count[0] ),
        .y ( int_stridx_step_res )
    );

    stage_3_exe_eu_misc_imul_imm u_int_imul_imm (
        .a ( i_int_a ),
        .b ( i_int_b ),
        .y ( int_imul_imm_res ),
    ...
    stage_3_exe_eu_misc_clts u_int_clts (
        .cr0 ( i_int_a ),
        .y ( int_clts_res )
    );

    stage_3_exe_eu_misc_lmsw u_int_lmsw (
        .cr0 ( i_int_a ),
        .src ( i_int_b ),
        .y ( int_lmsw_res )
    );

    stage_3_exe_eu_misc_smsw u_int_smsw (
        .cr0 ( i_int_a ),
        .y ( int_smsw_res )
    );

    stage_3_exe_eu_misc_loop_ctrl u_int_loop_ctrl (
        .ecx ( i_int_a ),
        .zf ( i_int_b[0] ),
    ...
    stage_3_exe_eu_misc_flag_status u_int_flag_status (
        .flags_in ( i_int_a ),
        .op ( i_int_op ),
    ...
    stage_3_exe_eu_execute_x87_fpu u_x87 (
        .clk         ( clk ),
        .rst         ( rst ),
        .i_valid     ( i_x87_valid ),
        .i_op        ( i_x87_op ),
